...
def __getattr__(name):
    if name == 'projection':
        warnings.warn("The 'projection' module was renamed to 'projection' "
                      "to avoid confusion with the projection() function.",
                      DeprecationWarning)
        return projection
...
def projection(geom, projection1, projection2):
    """Reprojection shapely geometry object(s) or scalar
    ...
    projection = partial(transformer.transform)

    # do the transformation!
    if is_sequence(geom) and not isinstance(geom, BaseMultipartGeometry):
        return [transform(projection, g) for g in geom]
...
def projection_raster(source_raster, dest_raster, dest_crs,
                   resampling=1, resolution=None, num_threads=2,
                   driver='GTiff'):
    """Reprojection a raster from one coordinate system to another using Rasterio
    code from: https://github.com/mapbox/rasterio/blob/master/docs/reprojection.rst
    ...
    dest_raster : str
        Filename of reprojectioned (destination) raster. Extension of filename should
    ...
    dest_crs : obj
        ...
        Can be any of:
          ...
          - A :class:`pyproj.crs.CRS` class
    resampling : int
        Type of resampling to use when reprojectioning the raster
    ...
    from rasterio.warp import calculate_default_transform, reprojection

    with rasterio.open(source_raster) as src:
        print('reprojectioning {}...\nfrom:\n{}, res: {:.2e}, {:.2e}\n'.format(
                source_raster,
                src.crs.to_string(),
                src.res[0], src.res[1],
                dest_crs), end='')
