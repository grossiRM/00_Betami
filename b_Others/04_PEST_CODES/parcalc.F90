!     Last change:  J    16 Jun 2005   10:40 pm
include 'inter.inc'
include 'utility.inc'
include 'modio.inc'
...
! -- Program PARCALC calculates model parameter values from super parameter values.
...
interface
	...
	subroutine write_message(increment,iunit,error,leadspace,endspace)
	  ...
	  character (len=*), intent(in), optional ::error,leadspace,endspace
        ...
        integer              :: ntpfle,i,ierr,j,npar,nsup,ifail,    &
                                ies,nespar,iiflag,nnfle,nlscaladj,nn,jfail
        integer              :: npnt
        integer              :: lw(5),rw(5)
        integer, allocatable :: itrans(:)
        character*8          :: precision,decpoint
        character*12         :: aversion
        character*12, allocatable :: apar(:)
        double precision     :: rtemp
        double precision, allocatable :: pval(:),sval(:),lbound(:),ubound(:), &
                                         value(:),svalue(:),eigmat(:,:),      &
                                         scale(:),offset(:)
        double precision,allocatable  :: pvals(:)
        double precision,allocatable  :: sf(:)
        character*200                 :: infile,supvalfile,comline
        character*200, allocatable    :: tplfle(:),infle(:)
        character*500                 :: dline

       include 'version.inc'
       write(6,10) trim(aversion)
10     format(' PARCALC Version ',a,'. Watermark Numerical Computing.',/)
...
! -- The parcalc input file is now read.
        ...
        read(10,*,err=9000,end=9100) npar,nsup,nespar,nlscaladj
        read(10,*,err=9000,end=9100) ntpfle
        allocate(apar(npar),pval(npar),sval(npar),itrans(npar),lbound(npar), &
        ubound(npar),scale(npar),offset(npar),pvals(npar),sf(npar),stat=ierr)
        if(ierr.ne.0) go to 9200
        allocate(value(nsup),svalue(nsup),stat=ierr)
        if(ierr.ne.0) go to 9200
        allocate(eigmat(nespar,nsup),stat=ierr)
        if(ierr.ne.0) go to 9200
        allocate(tplfle(ntpfle),infle(ntpfle),stat=ierr)
        if(ierr.ne.0) go to 9200
        read(10,*,err=9000,end=9100)
        do i=1,nsup
          read(10,*,err=9000,end=9100) value(i),svalue(i)
        end do
        read(10,*,err=9000,end=9100)
        do i=1,npar
          read(10,*,err=9000,end=9100) apar(i),itrans(i),sval(i),lbound(i),ubound(i), &
          scale(i),offset(i)
        end do
        read(10,*,err=9000,end=9100)
        iiflag=0
...
91        read(10,'(a)',err=9000,end=9100) dline
          ...
          call spacesub(dline)
          call linspl(jfail,2,lw,rw,dline)
          if(jfail.ne.0) go to 9000
          tplfle(i)=dline(lw(1):rw(1))
          infle(i)=dline(lw(2):rw(2))
          call remchar(tplfle(i),char(211))
          call remchar(infle(i),char(211))
          if(comline.ne.' ')then
            nn=npnt(infle(i))
            if(nn.lt.0)then
              infle(i)=trim(infle(i))//'.'//trim(comline)
            else
              infle(i)=infle(i)(1:nn)//trim(comline)//infle(i)(nn:)
            end if
          end if
          if(tplfle(i).eq.'picalc.tpl')iiflag=i
        end do
        read(10,*,err=9000,end=9100)
        do i=1,nespar
        ...
        if(nlscaladj.ne.0)then
          read(10,*,err=9000,end=9100)
          read(10,*,err=9000,end=9100) (sf(i),i=1,npar)
        end if
        close(unit=10)
        write(6,120) trim(infile)
120     format(' - file ',a,' read ok.')

! -- Model parameters are calculated from super parameters.

        ies=0
        do i=1,npar
          if(itrans(i).le.-100000) then
            pval(i)=sval(i)
          else if(itrans(i).ge.0)then
            ies=ies+1
            ...
            do j=1,nsup
              if(nlscaladj.eq.0)then
                rtemp=rtemp+eigmat(ies,j)*(value(j)-svalue(j))
              else
                rtemp=rtemp+eigmat(ies,j)*(value(j)-svalue(j))*sf(i)
              end if
            end do
...
!    parameters.
         ...
         do i=1,npar
           if(itrans(i).ge.0)then
             if(pval(i).ge.ubound(i))pval(i)=ubound(i)
             if(pval(i).le.lbound(i))pval(i)=lbound(i)
          end if
        end do
...
! -- The values of tied parameters are calculated.
        ...
        do i=1,npar
          if((itrans(i).lt.0).and.(itrans(i).gt.-100000)) then
            j=-itrans(i)
            pval(i)=pval(j)*sval(i)/sval(j)
          end if
        end do

! -- The scale and offset is enforced. But first a copy is made of these parameters
!    for writing to the PICALC input file which does regularisation based on PEST
!    parameters before scaling and offseting (because this is the way it is done in
!    the original base PEST control file.)

        do i=1,npar
          pvals(i)=pval(i)
          pval(i)=pval(i)*scale(i)+offset(i)
        end do

! -- Now parameters must be written to the model input files using template
!    files.

        write(6,124)
124     format(' Processing template files and writing model input files....')
        if(iiflag.eq.0)then
          nnfle=ntpfle
        else
          nnfle=ntpfle-1
        ...
        precision='single'
        decpoint='point'
        call initialise_mio(ifail,nnfle,1,npar,1,precision,decpoint)
        if(ifail.ne.0) go to 9400
        do i=1,ntpfle
          if(i.ne.iiflag)then
            call put_mio_file(ifail,1,i,tplfle(i))
            if(ifail.ne.0) go to 9400
            call put_mio_file(ifail,2,i,infle(i))
            if(ifail.ne.0) go to 9400
          end if
        end do
        call process_template_files(ifail,npar,apar)
        if(ifail.ne.0) go to 9400
        call write_model_input_files(ifail,npar,apar,pval,pvals,itrans)
        if(ifail.ne.0) go to 9400
        call finalise_mio()


! -- If picalc.tpl is a template file then we write that with non-scaled
!    parameters.

        if(iiflag.ne.0)then
          call initialise_mio(ifail,1,1,npar,1,precision,decpoint)
          if(ifail.ne.0) go to 9400
          call put_mio_file(ifail,1,1,tplfle(iiflag))
          if(ifail.ne.0) go to 9400
          call put_mio_file(ifail,2,1,infle(iiflag))
          if(ifail.ne.0) go to 9400
          call process_template_files(ifail,npar,apar)
          if(ifail.ne.0) go to 9400
          call write_model_input_files(ifail,npar,apar,pvals,pvals,itrans)
          if(ifail.ne.0) go to 9400
          call finalise_mio()
        end if
        write(6,125)
125     format(' - model input files written ok.')
        go to 9900
...
9110    format('*** Premature end to PARCALC input file ',a,' ***')
        go to 9890
9200    write(amessage,9210)
9210    format('*** Cannot allocate sufficient memory for PARCALC ',  &
        'execution ***')
        go to 9890
9400    write(6,9410)
...
9900    continue
        deallocate(itrans,apar,pval,sval,lbound,ubound,value,svalue, &
        eigmat,tplfle,infle,scale,offset,pvals,sf,stat=ierr)
